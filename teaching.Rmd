...
Loss function is a "scanning apparatus".  
It will take special value at true parameter.

Such as $\rho$ or minimum point.

## Mean Squared loss

## Kullback-Leibler Divergence

### Empirical Loss and MLE

Consider the empirical loss function：

$$L(\theta) = \mathbb{E}_{\Theta} \left[ \frac{f(x;\theta)}{f(x;\theta)} \log \frac{f(x;\theta)}{f(x;\theta)} \right]$$

$$= \int \frac{f(x;\theta)}{f(x;\theta)} \log \frac{f(x;\theta)}{f(x;\theta)} f(x;\theta) dx$$

$$= \int \left[ \log \frac{f(x;\theta)}{f(x;\theta)} \right] f(x;\theta) dx$$

$$= \mathbb{E}_{\Theta} \left[ \log \frac{f(x;\theta)}{f(x;\theta)} \right]$$

$$\approx \frac{1}{n} \sum_{i=1}^{n} \log \frac{f(x_i;\theta)}{f(x_i;\theta)}$$

$$= \frac{1}{n} \sum_{i=1}^{n} \log f(x_i;\theta) - \frac{1}{n} \sum_{i=1}^{n} \log f(x_i;\theta)$$
...
设 $E_{\theta}$ 为关于 $f_{\theta}(x)$ 的期望。

真实模型为 $\theta_0$，比较模型为 $\theta$。

考虑 KL 散度：

$$\mathbb{E}_{\theta} \left[ \frac{f(x;\theta_0)}{f(x;\theta)} \log \frac{f(x;\theta_0)}{f(x;\theta)} \right]$$

令 $\varphi(x) = x \log x$，则 $\varphi'(x) = \log x + 1$，$\varphi''(x) = \frac{1}{x} > 0$，  
因此 $\varphi(x)$ 在 $(0,+\infty)$ 上是凸函数。
...
DeepSeek loss function---喂数据---梯度下降---LLM
